...
import './App.css'
import { FpsCounter, recordFrame } from './components/FpsCounter';
// import CameraPreview from "./components/CameraPreview";
// import type { CaptureData } from "./components/CameraPreview";
...
async function renderResult(input: string | ImageData, results: any) {
  if (!results) return;
...
async function render() {
  ...
  const results = await runOnnxModel(imageData);
  await renderResult(imageData, results.output0);
  recordFrame();
...
function App() {
  ...
  // const imageUrl = 'https://images.unsplash.com/photo-1543466835-00a7907e9de1?q=80&w=640&auto=format&fit=crop';
  ...
  useEffect(() => {
    (async () => {
      if (smartphone_camera === undefined) {
        ...
        const results = await runOnnxModel(imageUrl);
        setInferenceResult(JSON.stringify(results));
        await renderResult(imageUrl, results.output0);
